...
{
	int sec;
	secounter += dt/((double)NUM_CIRCLES * 4.0);
...
update_swarmer(swarmer_t* s, double dt)
{
	...
	if (dt) {
		...
		double dx, dy;
		dx = s->dx + next_random()*0.05;
		dy = s->dy + next_random()*0.05;
		if (s->circle->on && s->distance < CIRCLE_RADIUS*1.3) {
			...
			dx += (s->circle->x - s->x) * 0.2;
			dy += (s->circle->y - s->y) * 0.2;
		}
		if (nearest_friend < 0.1) {
			dx -= (friend->x - s->x) * 0.2;
			dy -= (friend->y - s->y) * 0.2;
		}
		else if (nearest_friend < 0.5) {
			dx += (friend->x - s->x) * 0.2;
			dy += (friend->y - s->y) * 0.2;
		}
		...
		if (s->circle->on) {
			if (nearest_enemy < 0.2) {
				dx += (enemy->x - s->x) * 0.05;
				dy += (enemy->y - s->y) * 0.05;
			}
		} else {
			if (nearest_enemy < 0.8) {
				dx += (enemy->x - s->x) * 0.1;
				dy += (enemy->y - s->y) * 0.1;
			}   
		}
...
draw_swarmer(cairo_t* cr, gint width, gint height, swarmer_t* s)
{
	...
	cairo_arc(cr, s->x*width, s->y*height, 1.0 + agelim*2.0, 0.0, 2 * M_PI);
	cairo_fill(cr);
}
